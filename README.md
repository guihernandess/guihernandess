...
| **Ferramentas e Versionamento:** | `Git` `GitHub` `VS Code` `Visual Studio` |
| **Idiomas:** | `Inglês Avançado (C1)` `Português Nativo` |

---

### 📈 Conhecimentos em Controle de Versão (Git/GitHub)
- Domínio do fluxo de trabalho Git: `clone`, `push`, `pull`, `log` e gestão de repositórios remotos.
- Experiência em resolução de conflitos de `merge` e versionamento integrado ao VS Code.
- Manipulação de histórico e recuperação de estados através dos comandos `amend`, `reset` e `revert`.
- Documentação técnica de projetos utilizando `README.md` e automação de arquivos ignorados via `.gitignore`.
